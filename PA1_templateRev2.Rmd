...
* What is total number of steps taken per day together with its mean and median values?
...
## Removing NA values  
Histograms representing the distribution of number of steps per interval for each of the 61 days, show a similar range in the frequency distribution values, as well as, in the number of steps per interval.The differences among steps per day, however, are more apparent if the log base 10 of the number of steps per interval are used:  
  
Figure 1  

```{r histo log steps per interaval per day, echo=FALSE, }
...
The number of steps per day are compared among the 53 days of measurments in the bar graph below:
```{r calculation of total number of steps per day, echo=FALSE}
plot.new()
par("mar")
...
Figure 2.  
```{r comparison of total number of steps per day, echo=FALSE}
...
The mean and median values for the number of steps per day were calculated from the sum of steps for all intervals in a given day, and its distribution of values is shown in the histogram below:  

```{r mean and median and histo per day, echo=FALSE}
steps.day <- aggregate(activity1$steps,by=list(activity1$date),sum)
...
```{r histogram of the mean of the total number of steps per day, echo=FALSE}
...
```{r mean and median per interval, echo=FALSE}
...
daily.activity <- aggregate(steps~interval, data=activity1, mean)
...
Figure 4.  
```{r daily activity plot, echo=FALSE}
...
With the 104th interval, corresponding to the 8:35am interval, showing the highest mean step activity from October to November 2012 for the given monitored individual as `r daily.activity[x,2]`.    
...
and a new data-set named "activity3" is based on random selection values method (RSVM) replacement imputation  

```{r Random value replacement imputation, echo=FALSE}
activity3 <- activity
...
First, in Figure 5. we see a set of histograms in terms of the distributions of the log 10 step numbers per interval per day in terms of the original data compared to the RSVM imputed data : 
  
Figure 5.  
```{r comparison histogram 1, echo=FALSE}
...
The use of the mean value imputation, in Figure 6., alters the distribution patterns for the days with missing values introducing a single biased mean value: 
  
Figure 6.  
```{r comparison histogram 2, echo=FALSE}
histogram(~log10(steps)|date, data=activity2,layout=c(7,9), main="Mean Imputed Dataset", par.strip.text=list(cex=.75))
```


Second, one can compare the distributions of steps per day between the original dataset and the RSV imputation dataset as shown in the plot below (Figure7.):  
  
Figure 7.

```{r comparison histogam per day, echo=FALSE}
...
the plot shows that, as would be expected, the total number of steps is increased in the RSV imputed data set compared to the original data set (we have added entries to the missing values). From the distributions, the RSV imputed data set seems to be a valid correction of missing values as seeen by the  distributions being very similar between the two data sets, although the RSV imputation has smoothed the original distribution eliminating some of the data-set's non-normal random characteristics.  

The R summary of the RSV imputed data set (activity3, Table 2) is compared to the summary of the NA removed original data set (activity1, Table 3):   
 
Table 2. Original Dataset
```{r summaries of original dataset, echo=FALSE}
summary (activity1)
```
  
Table 3. RSVM imputed dataset
```{r summaries of RSVM imputed datasets, echo=FALSE}
summary (activity3)
```
...
```{r mean and std table, echo=FALSE}
...
```{r processing time data, echo=FALSE}
...
```{r mean steps per interval par week day, echo=FALSE}
...
```{r day of the week data sets, echo=FALSE}
...
```{r compariative time series per week day, echo=FALSE}
plot.new()
...
```{r comparison means per day data sets, echo=FALSE}
...
```{r comparison weekday vs weekend, echo=FALSE}
...
```{r weekday weekend mean and sd table, echo=FALSE}
